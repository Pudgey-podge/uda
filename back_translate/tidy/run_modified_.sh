...
a separate line
'''
input_file=greater_than_300_example_with_punctuation_.txt
...
python punctuation_remover_python_2.py
...
t2t-decoder \
  ...
  --hparams_set=transformer_big \
  --hparams="sampling_method=random,sampling_temp=${sampling_temp}" \
  --decode_hparams="beam_size=1,batch_size=256,alpha=0" \
  --checkpoint_path=checkpoints/fren/model.ckpt-500000 \
  --output_dir=/tmp/t2t \
  --decode_from_file=french_with_no_punctuation.txt \
